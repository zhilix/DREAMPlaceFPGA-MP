...
//DBG
template <typename T>
void print_site_pq_top_contents(
        const int* site_pq_idx,
        const int* site_stable,
        const T* site_pq_score,
        const int* site_pq_siteId,
        const int* site_pq_sig_idx,
        const int* site_pq_sig,
        const int* site_pq_impl_lut,
        const int* site_pq_impl_ff,
        const int* site_pq_impl_cksr,
        const int* site_pq_impl_ce,
        const int site_id, const int sIdx, const int tsPQ,
        const int SIG_IDX, const int SLICE_CAPACITY,
        const int CKSR_IN_CLB, const int CE_IN_CLB)
{
    std::cout << "Site: " << site_id << " has " << site_pq_idx[sIdx] << " candidates in PQ stable for "
        << site_stable[sIdx] << " iterations" << std::endl;
    ...
    std::cout << "Score: " << site_pq_score[tsPQ] << " and siteId: " << site_pq_siteId[tsPQ] << std::endl;

    std::cout << "There are " << site_pq_sig_idx[tsPQ] << " elements in sig: ";
    for(int sg = 0; sg < SIG_IDX; ++sg)
    {
        std::cout << site_pq_sig[topIdx + sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "LUTs: ";
    for(int sg = 0; sg < SLICE_CAPACITY; ++sg)
    {
        std::cout << site_pq_impl_lut[lutIdx + sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "FFs: ";
    for(int sg = 0; sg < SLICE_CAPACITY; ++sg)
    {
        std::cout << site_pq_impl_ff[lutIdx + sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "CKSR: ";
    for(int sg = 0; sg < CKSR_IN_CLB; ++sg)
    {
        std::cout << site_pq_impl_cksr[ckIdx + sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "CE: ";
    for(int sg = 0; sg < CE_IN_CLB; ++sg)
    {
        std::cout << site_pq_impl_ce[ceIdx + sg] << " ";
    }
    std::cout << std::endl;
}
...
void print_cand(
        const int SIG_IDX, const int SLICE_CAPACITY,
        const int CKSR_IN_CLB, const int CE_IN_CLB,
        const T nwCand_score, const int nwCand_siteId,
        const int nwCand_sigIdx, const int* nwCand_sig,
        const int* nwCand_lut, const int* nwCand_ff,
        const int* nwCand_cksr, const int* nwCand_ce)
{
    std::cout << "Score: " << nwCand_score << " and siteId: " << nwCand_siteId << std::endl;

    std::cout << "There are " << nwCand_sigIdx << " elements in sig: ";
    for(int sg = 0; sg < SIG_IDX; ++sg)
    {
        std::cout << nwCand_sig[sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "LUTs: ";
    for(int sg = 0; sg < SLICE_CAPACITY; ++sg)
    {
        std::cout << nwCand_lut[sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "FFs: ";
    for(int sg = 0; sg < SLICE_CAPACITY; ++sg)
    {
        std::cout << nwCand_ff[sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "CKSR: ";
    for(int sg = 0; sg < CKSR_IN_CLB; ++sg)
    {
        std::cout << nwCand_cksr[sg] << " ";
    }
    std::cout << std::endl;
    std::cout << "CE: ";
    for(int sg = 0; sg < CE_IN_CLB; ++sg)
    {
        std::cout << nwCand_ce[sg] << " ";
    }
    std::cout << std::endl;
}
...
inline void compute_candidate_score(const T* pos_x, const T* pos_y, const T* pin_offset_x, const T* pin_offset_y, const T* net_bbox, const T* net_weights, const int* net_pinIdArrayX, const int* net_pinIdArrayY, const int* flat_net2pin_start_map, const int* flat_node2pin_start_map, const int* flat_node2pin_map, const int* sorted_net_map, const int* pin2net_map, const int* pin2node_map, const int* net2pincount, const T* site_xy, const int* node_names, const T xWirelenWt, const T yWirelenWt, const T extNetCountWt, const T wirelenImprovWt, const int netShareScoreMaxNetDegree, const int wlScoreMaxNetDegree, const int* res_sig, const int res_siteId, const int res_sigIdx, T &result)
{
    ...
    ////DBG
    //std::cout << "Score: " << result << " = " << netShareScore << "(netShareScore) + "
    //    << wirelenImprovWt << "(wirelenImprovWt) * " << wirelenImprov << "(wirelenImprov)"
    //    << std::endl;
    //std::cout << "numNets: " << numNets << " and numIntNets = " << numIntNets << std::endl;
    ////DBG
}
...
#pragma omp parallel for num_threads(num_threads) schedule(dynamic, chunk_size)
    for(int sIdx = 0; sIdx < num_clb_sites; ++sIdx)
    {
        ...
        {
            ////DBG
            //print_site_pq_top_contents(site_curr_pq_idx, site_curr_stable, site_curr_pq_score,
            //        site_curr_pq_siteId, site_curr_pq_sig_idx, site_curr_pq_sig, site_curr_pq_impl_lut,
            //        site_curr_pq_impl_ff, site_curr_pq_impl_cksr, site_curr_pq_impl_ce,
            //        siteId, sIdx, tsPQ, SIG_IDX, SLICE_CAPACITY, CKSR_IN_CLB, CE_IN_CLB);
            ////DBG
            ...
            for(int iSig = 0; iSig < site_det_sig_idx[sIdx]; ++iSig)
            {
                ...
                ////DBG
                //std::cout << "Site: " << siteId << " assigned to inst_next_detSite of inst: " << dbgInstId << std::endl; 
                ////DBG
            }
                ...
                if (site_curr_scl_validIdx[siteCurrIdx] != INVALID)
                {
                    ...
                    for (int snIdx = 0; snIdx < site_nbr_idx[sIdx]; ++snIdx)
                    {
                                ...
                                add_inst_to_cand_impl(lut_type, flat_node2pin_start_map, flat_node2pin_map, node2pincount, net2pincount, pin2net_map, pin_typeIds, sorted_net_map, flat_node2prclstrCount, flat_node2precluster_map, flop2ctrlSetId_map, node2fence_region_map, flop_ctrlSets, instId, CKSR_IN_CLB, CE_IN_CLB, HALF_SLICE_CAPACITY, NUM_BLE_PER_SLICE, nwCand.impl_lut, nwCand.impl_ff, nwCand.impl_cksr, nwCand.impl_ce))
                        {
                            ...
                            {
                                ////DBG
                                //    std::cout << "nwCand assigned to site_next_pq" << std::endl;
                                //    print_cand(SIG_IDX, SLICE_CAPACITY, CKSR_IN_CLB, CE_IN_CLB,
                                //            nwCand.score, nwCand.siteId, nwCand.sigIdx, nwCand.sig,
                                //            nwCand.impl_lut, nwCand.impl_ff, nwCand.impl_cksr, nwCand.impl_ce);
                                ////DBG
