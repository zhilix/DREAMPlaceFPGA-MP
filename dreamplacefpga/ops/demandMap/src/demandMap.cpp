...
#define CALL_FPGA_LAUNCHER(atomic_add_op, map_ptr0, map_ptr2, map_ptr3, map_ptr4)  \
  computeDemandMapLauncher<scalar_t, decltype(atomic_add_op)>(           \
      ...
      num_threads, atomic_add_op, map_ptr0, map_ptr2, map_ptr3, map_ptr4)
        ...
        int deterministic_flag)
{
    ...
    DREAMPLACE_DISPATCH_FLOATING_TYPES(node_size_x, "computeDemandMapLauncher", [&] {
            if (deterministic_flag == 1)
            {
                ...
                CALL_FPGA_LAUNCHER(atomic_add_op, buf0.data(), buf2.data(), buf3.data(), buf4.data());
            ...
            } else
            {
                ...
                CALL_FPGA_LAUNCHER(atomic_add_op, buf0, buf2, buf3, buf4);
            }
